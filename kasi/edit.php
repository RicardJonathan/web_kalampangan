...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($password)) {
        $update_query = "UPDATE kasi SET nama='$nama', nip='$nip', jabatan='$jabatan', pangkat='$pangkat', golongan='$golongan', username='$username', password='$password' WHERE id='$id'";
    } else {
        $update_query = "UPDATE kasi SET nama='$nama', nip='$nip', jabatan='$jabatan', pangkat='$pangkat', golongan='$golongan', username='$username' WHERE id='$id'";
    }
    
    if (mysqli_query($koneksi, $update_query)) {
        // Jika berhasil, reload halaman untuk menampilkan data terbaru
        header("Location: profile_kasi.php");
        exit();
    } else {
