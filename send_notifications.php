...
// Fungsi bantu untuk menyimpan status notifiadmin ke kolom notif_terkirim
...
while ($row = mysqli_fetch_assoc($result_pengajuan)) {
    // Cek apakah notifikasi untuk status "Diajukan" sudah terkirim
    $notifikasi = json_decode($row['notif_terkirim'], true);
    if (is_array($notifikasi) && in_array('Diajukan', $notifikasi)) {
...
foreach ($pengajuan_ids as $id) {
    tambahStatusNotifTerkirim($koneksi, $id, 'Diajukan');
}
